...
/**
 * 관리자 화면의 단일 공용 레이아웃.
 * 팀원이 만든 features/admin 경로를 기준으로 유지하고 모든 관리자 기능을 같은 사이드바에 연결한다.
 */
  ...
  return (
    <div className="min-h-screen bg-subtle lg:grid lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="bg-navy-900 px-5 py-7 text-white lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
        ...
        <nav className="mt-5 flex gap-2 overflow-x-auto lg:mt-0 lg:block lg:space-y-2" aria-label="관리자 메뉴">
          {navigation.map((item) => (
            <NavLink
              className={({ isActive }) =>
        ...
        <div className="mt-8 border-t border-gold-300/60 pt-4 text-[11px] leading-5 lg:mt-auto">
          ...
          <p className="mt-2 text-white/85">{adminEmail}</p>
        </div>
      </aside>
      <div className="min-w-0">
        <header className="flex h-20 items-center justify-between border-b border-border-subtle bg-white px-6 lg:px-12">
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-navy-900">{sectionTitle(location.pathname)}</p>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:block">
              <p className="text-xs font-medium text-navy-900">{adminName}</p>
              <p className="mt-0.5 text-[10px] text-ink-700">{adminEmail}</p>
            </div>
          </div>
        </header>
        <Outlet />
